...
| Repository publication | [Public repository checklist](PUBLIC_REPOSITORY_CHECKLIST.md) | Resolve license/provenance decisions, add repository safeguards and contributor docs, then audit the publication checkpoint and GitHub controls | Repository visibility and history require review beyond a public player package |
...
| Public player/package | [Public release review](PUBLIC_RELEASE_REVIEW.md) | Build the silent `runtime-public` target in Release, package, audit, then follow the deployment document | Packaging/deployment identity is separate from gameplay admission |
