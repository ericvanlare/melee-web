...
| Browser player or menu | [Accuracy contract](ACCURACY_CONTRACT.md) | Run `node scripts/browser_smoke.mjs ...` for readiness; use `runtime.html` through the original CSS/SSS route and the shared `scripts/browser_driver.mjs` helper in automation. Summarize an existing failed replay with [Browser failure triage](BROWSER_FAILURE_TRIAGE.md). | Browser exercised; menu/transition and input claims need their own comparison |
