...
| DSP direct-mode phase | The original AX DSP clears saved phase at a 32-sample direct-mode block boundary; the port currently retains it. The [audio replacement](AUDIO_REPLACEMENT_EVIDENCE.md) preserves the existing port behavior | Compare a focused direct-to-filtered transition against the original DSP and resolve the deviation in a separate behavioral change |
