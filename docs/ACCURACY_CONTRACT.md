...
| Scene-entry preparation | CSS, SSS and match owner construction runs in a visible preparation phase with audio acknowledgement; source simulation remains stopped while unchanged frames discover uploads and drain pipelines through two quiet callbacks, and later first-use render work uses the same automatic gate | Repeat on clean application/driver caches, verify cold entry plus death/respawn without a manual hitch resume, and include full-match tails and audio underruns; CPU queue drainage is not a browser GPU completion fence |
