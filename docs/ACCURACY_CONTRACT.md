...
| --- | --- | --- |
| Native menu acceptance | Original CSS/SSS is the canonical player flow; source and browser diagnostic repeat loops pass, including original pause/resume, and raw PAD selects and renders Falco/Mario on Battlefield. A pinned read-only retail trace matches the port's CSS → SSS → CSS → SSS → match order and uninterrupted menu-audio ownership. The strict semantic comparison first diverges at SSS-exit match-data normalization | Align the port's match-data normalization boundary and identical retail fixture state, then repeat the pinned comparison and complete ordinary browser/physical input acceptance |
