...
window.menuSequenceMetrics=(options={})=>{
  ...
  return {
    ...
    source:{steps:nativeSourceSteps,draws:nativeSourceDraws,
      note:'Native source totals include preparation records; source_active and zero_source classify every valid native callback by source steps/draws; callbacks.live remains lifecycle timing classification.'},
    native:{
      callbacks:{total:nativeTimingFrames,live:nativeLiveTimingFrames,preparation:nativePreparationTimingFrames,source_active:nativeSourceActiveCallbacks,zero_source:nativeZeroSourceCallbacks,source_classified:nativeSourceActiveCallbacks+nativeZeroSourceCallbacks},
    ...
    browser:{
      ...
      longtasks:{supported:typeof PerformanceObserver==='function'&&PerformanceObserver.supportedEntryTypes?.includes('longtask')===true,
        count:browserLongTasks,worst_ms:browserLongTaskWorst,
        worst:sequenceMetricsCopy(browserLongTaskWorstRecord),
        records:sequenceMetricsCopy(browserLongTaskRecords),overflow:browserLongTaskOverflow},
    },
    ...
    diagnostic_capture:sequenceMetricsCopy(window.meleeHitchCapture?.report?.()||null),
  };
};
...
function replayMetrics(run){return{sourceFrames:run.consumed||Module._melee_web_native_menu_replay_cursor(),worstBrowserCallback,sourceSteps:nativeSourceSteps,sourceDraws:nativeSourceDraws,browserCallbacks:activeFrames,worstBrowserCallbackMs:worstFrame,browserCallbackGaps:longFrames,browserLongTasks,browserLongTaskWorstMs:browserLongTaskWorst,nativeCallbacks:nativeTimingFrames,nativeSourceActiveCallbacks,nativeZeroSourceCallbacks,worstNativeCallbackMs:nativeTimingWorst,nativeCallbacksOver33ms:nativeLongFrames,nativeCallbacksOverBudget:nativeOverBudgetFrames,worstNativeCallback:nativeWorstTiming,livePipelinesQueued:livePipelineQueued,livePipelinesCreated:livePipelineCreated,liveTextureUploadBytes:liveTextureUploads,liveStagingUsedBytes,peakStagingUsedBytes,preparationPauses:preparationCount-(run.baseline?.preparations??preparationCount),wasmHeapGrowthBytes:Module.HEAPU8.length-(run.baseline?.heap??Module.HEAPU8.length),focusLost:run.focusLost,firstFocusLoss:run.firstFocusLoss||null};}
