...
int assets_test_item_commands(void)
{
    ...
    const uint32_t effect[]={(10U<<26)|(0x155U<<16),
                             0x05130000,0x12345678,0x23456789,0x3456789a};
    commands=melee_web_item_commands_create(effect,5);
    if(!commands || commands[0].Command_00.code!=10 ||
       commands[0].Command_00.value!=0x155 ||
       ((uint32_t*)&commands[1])[0]!=0x00000513 ||
       ((uint32_t*)&commands[2])[0]!=0x56781234){
    ...
    const uint32_t nested_opcode[]={(16U<<26)|(32U<<18),0};
    commands=melee_web_item_commands_create(nested_opcode,2);
    if(!commands || commands[0].unk0.opcode!=16 || commands[0].unk0.unk1!=32){
        if(commands)melee_web_item_commands_destroy(commands);
        return 0;
    }
    melee_web_item_commands_destroy(commands);
