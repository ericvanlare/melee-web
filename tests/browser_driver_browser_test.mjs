...
<dialog id="error-dialog"><div id="error"></div></dialog>
...
$(ids[4]).onchange=()=>{if($(ids[0]).disabled)throw Error('Import before ready');$(ids[1]).disabled=false;};
...
try {
  ...
  for(const surface of ['development','public']) {
    ...
    try {
      ...
      await page.evaluate(()=>{document.querySelector('#error').textContent='Preparation rejected fixture';document.querySelector('#error-dialog').showModal();});
