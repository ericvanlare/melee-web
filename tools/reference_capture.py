...
class Created(gdb.Breakpoint):
 ...
 def stop(self):
  try:
   ...
   emit({'event':'Fighter_Create_return','gobj':hex(gobj),'fighter':hex(fp),'state':row})
 ...
 def stop(self):
  ...
  if enabled:
   try:
    ...
    emit({'event':'scheduler_return','tick':tick,'game_frame':word(0x8046B6C4),'phase':'HSD_GObj_80390CFC_return','grkind':struct.unpack_from('>I',stage,0x88)[0],'stage_camera_blast_hex':stage[:0x80].hex(),'fighters':rows})
