...
A match is not gold admission: the v1 probe excludes rendering, initial global
PAD histories and hardware/audio output. This tool cannot waive those boundaries.
"""
...
def validate_port(rows):
    ...
    fixed={'record':'header', 'schema':'melee-web-port-replay-candidate', 'version':1,
    ...
    _require_keys(rows[1], {'record','rng','start_melee_hex'}, 'port.match_enter')
    ...
    _require_keys(rows[2], STATE|{'record'}, 'port.initial')
    if rows[2]['record']!='match_enter_complete': raise CaptureError('port: missing initial state')
    _state(rows[2], 'port.initial')
    previous=0
    for index, row in enumerate(rows[3:-1]):
        _require_keys(row, STATE|{'record','index','supplied_inputs'}, f'port.frame[{index}]')
...
def compare_rows(reference, rows):
    ...
    report={'status':'declared_state_match', 'gold_admitted':False, 'performance':'not_evaluated',
            'scope':'declared entry data, supplied inputs, fighter fields, RNG and match clock only; '
    ...
    check('entry', {k:reference.match_enter[k] for k in ('rng','start_melee_hex')},
          {k:rows[1][k] for k in ('rng','start_melee_hex')}, 'match_enter')
    for group in ('rng','match_frame','fighters'):
    ...
    for group in ('frame_count','entry','inputs','rng','match_frame','fighters'):
