...
from retail_replay_validation import (
    ...
    MAX_FRAMES, PAD_STATE_BYTES, MULTIPLAYER_VERSION, _pad_state,
)
...
def _state(row, context, version=1, active_player_count=2):
    if version in (2, MULTIPLAYER_VERSION):
        _pad_state(row["pad_state_hex"], context+".pad_state_hex")
    _u32(row['rng'], context+'.rng')
    _u32(row['match_frame'], context+'.match_frame')
    if version == MULTIPLAYER_VERSION:
        _int(active_player_count, context + '.active_player_count', minimum=2, maximum=4)
    else:
        active_player_count = 2
    if not isinstance(row['fighters'], list) or len(row['fighters']) != active_player_count:
        raise CaptureError(context+f': exactly {active_player_count} fighters required')
...
def validate_port(rows):
    ...
    head=rows[0]
    version=_int(head["version"], "port.version", minimum=1, maximum=MULTIPLAYER_VERSION)
    header_keys = HEADER | ({"active_player_count"} if version == MULTIPLAYER_VERSION else set())
    _require_keys(head, header_keys, 'port.header')
    active_player_count = (_int(head["active_player_count"], "port.active_player_count",
                                 minimum=2, maximum=4)
                           if version == MULTIPLAYER_VERSION else 2)
    state_keys=STATE|({"pad_state_hex"} if version in (2, MULTIPLAYER_VERSION) else set())
    ...
    _require_keys(rows[1], {'record','rng','start_melee_hex'}|
                  ({'pad_state_hex'} if version in (2, MULTIPLAYER_VERSION) else set()),
                  'port.match_enter')
    ...
    if version in (2, MULTIPLAYER_VERSION): _pad_state(rows[1]['pad_state_hex'], 'port.entry.pad_state_hex')
    ...
    _state(rows[2], 'port.initial', version, active_player_count)
    previous=0
    for index, row in enumerate(rows[3:-1]):
        ...
        _state(row, f'port.frame[{index}]', version, active_player_count)
...
def compare_rows(reference, rows):
    ...
    groups=("rng","match_frame","fighters")+(("pad_state_hex",)
                                                   if version in (2, MULTIPLAYER_VERSION)
                                                   else ())
    report={'status':'declared_state_match', 'gold_admitted':False, 'performance':'not_evaluated',
            ...
            'scope':'declared entry data, supplied inputs, fighter fields, RNG and match clock'+
                    (', PAD configuration and all three history banks' if version in (2, MULTIPLAYER_VERSION) else '')+' only; '+
                    ('rendering, hardware and audio output are excluded' if version in (2, MULTIPLAYER_VERSION) else
    ...
    if version == MULTIPLAYER_VERSION:
        check('active_player_count',
              {'active_player_count': reference.header['active_player_count']},
              {'active_player_count': rows[0]['active_player_count']}, 'header')
    check('frame_count', len(reference.frames), len(rows)-4, 'header')
    check('entry', {k:reference.match_enter[k] for k in ('rng','start_melee_hex')+
                    (("pad_state_hex",) if version in (2, MULTIPLAYER_VERSION) else ())},
          {k:rows[1][k] for k in ('rng','start_melee_hex')+
           (("pad_state_hex",) if version in (2, MULTIPLAYER_VERSION) else ())}, 'match_enter')
    for group in groups:
    ...
    for group in (('active_player_count',) if version == MULTIPLAYER_VERSION else ()) + ('frame_count','entry','inputs')+groups:
