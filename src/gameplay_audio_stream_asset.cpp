...
GameplayAudioStream::GameplayAudioStream(MeleeWebAudio* audio,const char* path,std::span<const uint8_t> bytes):decoded_(bytes){
 ...
 for(const auto& b:decoded_.blocks){MeleeWebAudioStreamBlock out{};out.offset=b.file_offset;out.size=b.payload_size;out.end=b.end_nibble;out.next=b.next_offset;for(unsigned i=0;i<input_.channels;i++)out.channel[i]=channel(b.channels[i]);blocks_.push_back(out);}
