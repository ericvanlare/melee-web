...
/* joint_form is 0 for an owned model, 1 for the authored null/zero-bone form,
 * and 2 for the source Random Pokémon null-joint identity-root form (one bone,
 * attachment zero). Null forms still require an authored model descriptor;
 * the source scalar flags and bounds are preserved. */
int melee_web_article_publish(const MeleeWebNativeDat*,void* article,void* special,
    ...
    int joint_form,char* error,size_t error_size);
